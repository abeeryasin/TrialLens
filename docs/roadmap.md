...
| 7b | **What replaces it** — amendment history, the watch, the watch record (agreed 2026-09-01). See `docs/plan_after_ranking.md` | **In progress.** Unit 0 (removal) and **direction 1 (amendment history) done 2026-09-01/02** — `GET /studies/{nct_id}/amendments`, `api/amendments.py`, Understand leads with it, and `has_results` added after finding the parser never read it. **Direction 2 (the watch) is designed, not built** — `design/`, Streamlit-shaped on purpose. **Direction 3 (the watch record / `monitor_runs`) not started.** 201 tests; CI and a README exist from 2026-09-02 | 6-8 |
