...
| 7b | **What replaces it** — amendment history, the watch, the watch record (agreed 2026-09-01). None needs a model. See `docs/plan_after_ranking.md` | **In progress** (2026-09-01). Unit 0 (the removal) done. **Next: amendment history**, deliberately first because it is a build problem rather than a design problem — `study_changes` already holds the data, so it needs no new judgment calls, and after a dead end the right next thing is one with a known shape. The watch is second and is a design problem (what a quiet week should look like); the `monitor_runs` heartbeat is third | 6-8 |
