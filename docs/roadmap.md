...
**Where to pick up (2026-09-04).** The graph is built and **nobody can see
it**. `api/main.py` registers studies, discover, changes and watch — there is
no `explore` router — and nothing in `api/` or `frontend/` reads the graph
tables. Unreachable in the database right now: 191,864 edges, 49,606 sites
with coordinates, 40,011 edges with a recruitment status, 7 prose
interpretations, plus `frontend/labels.format_site_status()` which is written
and called by nothing.

Three days of real work landed (7b, 7c, step 8 units 1-2b) and the app looks
identical, because it is identical. Next session should be
`GET /explore/{nct_id}` and `frontend/pages/4_Explore.py`, then surfacing the
prose interpretations in Understand — not unit 3. 332 tests pass.

**Step 8, Explore, is underway** — units 1 (shape), 2 (extraction) and 2b
(node ranking + site enrichment) are done; see the step 8 row above and
`docs/decisions.md`, 2026-09-03. Next is unit 3 (the merge step, against the
unmerged baseline now on file) and then the Explore endpoint and page.
