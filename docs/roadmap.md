...
is live and the capability grid's fifth card opens a real page. Two things
are still invisible, and one is a smaller version of the problem just fixed:

1. **The 7 prose interpretations** (step 7c) are stored and read by nothing.
   No page shows them. This is the same "built, paid for, unreachable" state
   Explore was in for three days, at 1/27,000th the size — Understand is
   where they belong.
2. **Unit 3, the merge**, is still open and now has a real page to judge it
   against. The question to answer first is empirical, not architectural:
   open Explore on a trial whose sites include a much-duplicated hospital
   and see whether the duplicates actually surface in the city rollup. If
   they do not, the merge stays deferred.
