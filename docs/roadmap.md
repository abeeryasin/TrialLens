...
| 7b | **What replaces it** — amendment history, the watch, the watch record (agreed 2026-09-01). See `docs/plan_after_ranking.md` | **In progress.** Unit 0 (ranking removal) and **direction 1 (amendment history) both done 2026-09-01/02** — `GET /studies/{nct_id}/amendments` groups a trial's changes into the amendments that caused them, keyed on CT.gov's own version stamp, and `api/amendments.py` says what each one did in plain language without a model. First HTTP-level tests in the repo; 142 tests. **Next: direction 2, the watch** (a design problem — what a quiet week should look like), then direction 3, the `monitor_runs` heartbeat | 6-8 |
