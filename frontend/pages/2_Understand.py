"""Understand: full detail for one trial.
...
ClinicalTrials.gov via our own DB), shown as source text with its own
uncertainty made explicit, never an inference dressed up as a fact.
"""
...
try:
    study = get(f"/studies/{nct_id}")
except ApiError as exc:
    if exc.status_code == 404:
        st.warning(
            f"{nct_id} isn't in our tracked database. TrialLens only stores "
            "full detail for trials it actively tracks — if this ID came "
            "from a live Discover result, there's no cached record to show "
            "here."
        )
